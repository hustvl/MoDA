...
"""MoDA v16 Triton kernels and helpers."""
...
@triton.jit
def parallel_moda_chunk_visible_fwd_kernel(
    q,
    k,
    v,
    o,
    ...
    for i_s in range(base_t_block_start, base_t_block_end, BS):
        ...
        b_s = tl.where(
            (o_q_base[:, None] >= o_k[None, :]) & m_k[None, :],
            b_s,
            float("-inf"),
        )
...
def parallel_moda_fwd(
    ...
    parallel_moda_chunk_visible_fwd_kernel[grid](
        q=q,
        k=k,
        v=v,
        o=o,
...
def parallel_moda_chunk_visible_fwd(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g_cumsum: torch.Tensor,
...
@triton.jit(do_not_specialize=["T"])
def parallel_moda_chunk_visible_bwd_kernel_dq(
    q,
    k,
    v,
    lse,
    ...
    if USE_DEPTH:
        ...
        for depth_col_start in range(depth_block_start, depth_block_end, BS):
            ...
            causal_mask = depth_row_ids[None, :] <= query_base_rows[:, None]
            mask_depth = (row_valid[:, None]) & col_valid[None, :] & causal_mask
...
@triton.jit
def parallel_attn_chunk_visible_bwd_kernel_dkv_depth(
    q,
    cached_k,
    cached_v,
    lse,
...
def parallel_moda_chunk_visible_bwd(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    o: torch.Tensor,
    ...
    parallel_moda_chunk_visible_bwd_kernel_dq[grid](
        q=q,
        k=k,
        v=v,
        lse=lse,
    ...
    if use_depth:
        assert NV == 1, "Depth dkv backward currently supports only NV == 1 (BV >= V)"
        parallel_attn_chunk_visible_bwd_kernel_dkv_depth[grid](
            q=q,
            cached_k=cached_k,
            cached_v=cached_v,
            lse=lse,
...
def naive_mixture_of_depth_causal_chunk_visible_ref(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    kd: Optional[torch.Tensor] = None,
        ...
        for h in range(Hq):
            ...
            for t_q_idx in range(T_q):
                ...
                if use_depth:
                    ...
                    chunk_start_bt = (base_t // chunk_size) * chunk_size

                    S = base_t - chunk_start_bt + 1
                    kd_sel = kd_bh[chunk_start_bt : base_t + 1].reshape(S * L, Kq)
                    vd_sel = vd_bh[chunk_start_bt : base_t + 1].reshape(S * L, Vdim)
                    logits_depth = (kd_sel @ q_vec) * scale
                    logits = torch.cat([logits_space, logits_depth], dim=0)
                else:
                    logits = logits_space
                ...
                if use_depth:
                    ...
                    w_depth = w[base_t + 1 :]
                    ctx_space = (w_space.unsqueeze(1) * v_bh[: base_t + 1]).sum(0)
                    ctx_depth = (w_depth.unsqueeze(1) * vd_sel).sum(0)
                    out[b, t_q_idx, h] = ctx_space + ctx_depth
                else:
                    out[b, t_q_idx, h] = (w.unsqueeze(1) * v_bh[: base_t + 1]).sum(0)
...
def test_once(
    ...
    accuracy_report(out_impl_f32, ref_out, "MoDA 16p0 Kernel O")
    accuracy_report(lse_impl_f32, ref_lse, "MoDA 16p0 Kernel LSE")
...
def main_speed_benchmark():
    ...
    def modav16p0_spatial_only_test():
    ...
    modav16p0_time = benchmark(
        modav16p0_triton_test, "MoDA v16.0 Triton Implementation"
    )
    modav16p0_spatial_only_time = benchmark(
        modav16p0_spatial_only_test, "Flash Attention (spatial only)"
    )

    print(f"MoDA v16.0 Triton: {modav16p0_time:.3f} ms")
    print(f"Flash Attention V2 (spatial only): {modav16p0_spatial_only_time:.3f} ms")
...
def run_case_chunk_visible(
    ...
    torch.manual_seed(seed)
    T_q = T_kv * moda_group_num
    print(f"\n=== {case_name} (chunk-visible) ===")
    print(
        f"B={B}, T_kv={T_kv}, T_q={T_q}, H={H}, K={K}, V={V}, L={L}, group={moda_group_num}, dtype={dtype}"
    )

    q = torch.randn(B, T_q, H, K, device=device, dtype=dtype)
    k = torch.randn(B, T_kv, H, K, device=device, dtype=dtype)
    v = torch.randn(B, T_kv, H, V, device=device, dtype=dtype)

    if L > 0:
        kd = torch.randn(B, T_kv * L, H, K, device=device, dtype=dtype)
        vd = torch.randn(B, T_kv * L, H, V, device=device, dtype=dtype)
    else:
        kd = None
        vd = None
    ...
    out_kernel, lse_kernel = parallel_moda_chunk_visible_fwd(
        ...
        cached_v=vd,
        moda_group_num=moda_group_num,
        customized_BT=BT_eff,
        customized_BS=customized_BS,
        customized_BK=customized_BK,
        customized_BV=customized_BV,
    ...
    metric("Forward/O", out_kernel, out_ref)
    metric("Forward/LSE", lse_kernel, lse_ref)
    ...
    out_kernel_bwd, lse_kernel_bwd = parallel_moda_chunk_visible_fwd(
        q=q_kernel,
        k=k_kernel,
        v=v_kernel,
        g_cumsum=None,
    ...
    grad_out = torch.randn_like(out_kernel_bwd)

    dq, dk, dv, _, d_kd, d_vd = parallel_moda_chunk_visible_bwd(
        q=q_kernel,
        k=k_kernel,
        v=v_kernel,
        o=out_kernel_bwd,
...
if __name__ == "__main__":
    ...
    run_case_chunk_visible(
        "ChunkVisible Case1: no depth, group=1",
        B=2,
        T_kv=4096,
        H=4,
        K=64,
    ...
    run_case_chunk_visible(
        "ChunkVisible Case2: depth L=4, group=1",
        B=2,
        T_kv=4096,
        H=4,
        K=64,
    ...
    run_case_chunk_visible(
        "ChunkVisible Case3: depth L=4, group=2",
        B=2,
        T_kv=4096,
        H=4,
        K=64,
