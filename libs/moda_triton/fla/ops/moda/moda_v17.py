...
"""MoDA Triton kernels and reference helpers (v17, K1 variant).

Forked from ``moda_v14.py``. The only semantic change is the addition of a
``current_depth`` (a.k.a. ``L_cur``) parameter that is threaded through the
public API, the autograd Function, the Python wrappers, and all three
``USE_DEPTH``-aware kernels (forward, dQ backward, dKV-depth backward).

Motivation
----------
v14 expects a tightly packed depth-cache buffer ``[B, T_kv*L, H, K]`` where
``L`` is the *current* number of depth slots per token. Building this buffer
incrementally across ``L`` transformer blocks costs ``O(L^2)`` Python-side
allocator/copy bandwidth (a fresh ``torch.stack``/``torch.cat`` per block).

v17 lets the caller pre-allocate a max-depth buffer ``[B, T_kv*L_max, H, K]``
once, write a single new ``[B, T_kv, H, K]`` slot per block (an ``O(BNHK)``
scatter), and pass ``current_depth=L_cur`` to tell the kernel that only the
first ``L_cur`` slots per token are valid. The kernel still strides at
``L_max`` (so the pre-allocated buffer's layout is correct), but masks out
slot positions ``slot_in_token >= L_cur`` from the attention softmax. This
drops the per-block cache-management cost from ``O(L_cur*BNHK)`` to
``O(BNHK)``, totalling ``O(L*BNHK)`` instead of ``O(L^2*BNHK)``.

When ``current_depth`` is omitted (or set to ``L_max``) the kernel behaves
identically to v14 and consumes a tightly packed buffer as before, so this
file is a drop-in replacement.

Caveats
-------
* The kernel still iterates over the full ``L_max`` slot range per token and
  masks invalid positions; the wasted compute is at most
  ``(L_max - L_cur) / BS`` extra per-token positions, dominated by the
  baseline padding waste of v14.
* Gradients written to ``d_cached_k`` / ``d_cached_v`` for invalid slots are
  exactly zero (because the corresponding softmax weights are zero), so
  callers can read those positions safely; they do not need to slice off the
  invalid suffix.
* **The pre-allocated buffer must hold finite values at every position**
  (``torch.zeros`` is recommended; ``torch.empty`` is unsafe). The kernel
  computes ``tl.dot(b_q, b_k_depth)`` over the full ``L_max`` strip before
  masking with ``tl.where(..., -inf)``; if the unwritten suffix contains
  NaN/Inf the dot can leak NaN into masked-out lanes via tensor-core paths,
  which propagates through the subsequent ``tl.max`` reduction. Once a
  finite (or zero) garbage value is in the unused slot, the mask correctly
  zeroes its softmax contribution.
"""
...
def parallel_moda_fwd_kernel(
    ...
    cached_k,
    cached_v,
    L,        # L_max: depth dimension of the cached_k/v buffer per token
    L_cur,    # number of valid depth slots per token (L_cur <= L)
    moda_group_num,
    scale,
    cu_seqlens,
    chunk_indices,
    ...
    if USE_DEPTH:
        ...
        # Iterate over the *full* L_max-strided depth range so that block_ptr
        # addressing matches the pre-allocated buffer layout. Slots beyond
        # ``slot_in_token < L_cur`` are masked out below (the dot-product is
        # still computed on garbage values from the unwritten suffix of the
        # buffer, but ``tl.where(..., -inf)`` zeroes their softmax weight, so
        # the output is bit-identical to v14 with a tightly packed L_cur cache).
        ...
        for depth_col_start in range(depth_block_start, depth_block_end, BS):
            ...
            slot_in_token = depth_col_ids - depth_row_ids * L
            ...
            slot_valid_mask = slot_in_token < L_cur

            mask_depth = (
                row_mask & col_mask & row_match_mask & slot_valid_mask[None, :]
            )
...
def parallel_moda_fwd(
    ...
    if cached_k is not None and cached_v is not None:
        ...
        L_cur = L if current_depth is None else int(current_depth)
        assert 0 <= L_cur <= L, (
            f"current_depth={current_depth} must lie in [0, L_max={L}]"
        )
...
def parallel_moda_bwd_kernel_dq(
    ...
    cached_k,
    cached_v,
    L,        # L_max: depth dim of cached_k/v buffer per token
    L_cur,    # number of valid depth slots per token (L_cur <= L)
    moda_group_num,
    scale,
    cu_seqlens,
    ...
    if USE_DEPTH:
        ...
        # See forward-kernel comment: iterate the full L_max range (so
        # block_ptr addressing matches the pre-allocated buffer) and mask out
        # slot positions beyond ``L_cur`` per token.
        depth_block_start = base_rows_start * L
        depth_block_end = base_rows_end * L
        ...
        for depth_col_start in range(depth_block_start, depth_block_end, BS):
            ...
            slot_valid = slot_in_token < L_cur
...
def parallel_moda(
    ...
    """K1 variant of MoDA's parallel attention, see module docstring.

    ``current_depth`` (a.k.a. ``L_cur``) controls how many depth slots per
    token from the ``cached_k`` / ``cached_v`` buffer are valid. The buffer
    is interpreted as having ``L_max = cached_k.shape[1] // T_kv`` slots per
    token, with the first ``L_cur`` valid and the rest masked. When
    ``current_depth`` is ``None`` the kernel uses ``L_cur = L_max`` and is
    bit-identical to v14.
    """
    ...
    if use_depth:
        ...
        if current_depth is not None:
            assert 0 <= int(current_depth) <= L, (
                f"current_depth={current_depth} must lie in [0, L_max={L}] "
                f"(cached_k second dim is T_kv*L_max = {T_kv}*{L})"
            )
        if (
        ...
        warnings.warn(
            "current_depth is ignored when cached_k/cached_v are not provided."
        )
