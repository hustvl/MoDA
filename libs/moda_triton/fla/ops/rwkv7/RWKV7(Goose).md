...
dmoda = torch.sum(dstate_curr * b_t[None, :], dim=1)
da[bi, hi, t] += torch.sum(prev_state * dmoda[:, None], dim=0)
```
...
```python
dstate_from_sa = a_t[None, :] * dmoda[:, None]
    ...
    for t in range(L-1, -1, -1):
        for bi in range(B):
            for hi in range(H):
                ...
                db[bi, hi, t] = torch.sum(dstate_curr * sa[:, None], dim=0)
                dmoda = torch.sum(dstate_curr * b_t[None, :], dim=1)  # [V]

                # sa = (prev_state * a_t[None, :]).sum(dim=1)
                da[bi, hi, t] = torch.sum(prev_state * dmoda[:, None], dim=0)
                dstate_from_sa = a_t[None, :] * dmoda[:, None]  # [V, K]
