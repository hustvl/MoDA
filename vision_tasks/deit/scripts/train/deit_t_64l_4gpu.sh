#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# DeiT-Tiny depth-scaling baseline (64-layer full-attention ViT) on ImageNet-1k.
#
# Trains the `deit_tiny_64l_patch16_224` model registered in
# `vision_tasks/deit/models.py`. The factory mirrors
# `deit_tiny_patch16_224` (same ``embed_dim=192``, ``num_heads=3``,
# ``mlp_ratio=4``, ``patch_size=16``, ``img_size=224``) but stacks 64
# transformer blocks instead of 12 so that plain causal-free full-attention
# ViTs can be compared against the GQA / MoDA 64-layer variants trained by
# the sibling scripts in this directory.
#
# Companion BlaGPT depth-scaling recipe for reference:
#   language_tasks/BlaGPT/scripts/debug/baseline_depth_scaling.sh
#   (64-layer post-norm GPT-2 baseline, shares the 64L depth-scaling goal
#   but on LM data / Muon / SwiGLU. Architecture is separate; we only
#   match the "64 layers, everything else = 12L baseline" recipe idea.)
#
# Settings (CLI flags + main.py / timm defaults):
#   * Model      (deit_tiny_64l_patch16_224)
#       - embed_dim          = 192
#       - num_heads          = 3          (standard MHA, QKV = full)
#       - depth              = 64         (vs. 12 for deit_tiny_patch16_224)
#       - mlp_ratio          = 4
...
#
# Run:
#   bash vision_tasks/deit/scripts/train/deit_t_64l_4gpu.sh
#
...
PYTHON_BIN=python3
MODEL_NAME=deit_tiny_64l_patch16_224
OUTPUT_DIR=${OUTPUT_ROOT}/${MODEL_NAME}

cd "${DEIT_DIR}"
"${PYTHON_BIN}" -m torch.distributed.run --nproc_per_node=4 --master_port 49511 main.py --model "${MODEL_NAME}" --batch-size 256 --data-path "${DATA_PATH}" --output_dir "${OUTPUT_DIR}"
