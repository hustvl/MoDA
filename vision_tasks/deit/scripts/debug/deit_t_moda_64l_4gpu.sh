#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# DeiT-Tiny MoDA depth-scaling recipe (64-layer MoDA ViT) -- LOCAL DEBUG
# variant of ``scripts/train/deit_t_moda_64l_4gpu.sh``.
#
# Same model / CLI flags as the train script; the only differences are:
#   * No ``pip install`` setup block (assumes the deit conda env already
#     has ``fla`` from ``libs/moda_triton`` installed and triton==3.3.0).
...
#   * Model: ``deit_tiny_moda_64l_patch16_224`` (embed_dim=256, num_heads=4,
#     num_kv_heads=1, depth=64, mlp_ratio=4, patch16, img224,
#     moda_backend="v17", mlp_depth_kv_projection=True
#     -> 128-slot dual-slot-per-layer depth cache)
...
#
# Run:
#   bash vision_tasks/deit/scripts/debug/deit_t_moda_64l_4gpu.sh
# -----------------------------------------------------------------------------
set -euo pipefail
